...
Options:
    ...
    --dev               Include the utils folder in the export (for developer versions)
...
def create_terminal_games_zip(version, custom_games_dir=None, include_utils=False):
    """
    ...
    Args:
        ...
        include_utils (bool, optional): Whether to include the utils folder in the ZIP (for dev versions)
    ...
    with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        ...
        # Include utils folder if dev mode is enabled
        if include_utils:
            utils_dir = os.path.join(repo_root, 'utils')
            print(f"Developer mode: Including utils folder from {utils_dir}")
            
            # Create utils directory in the zip
            utils_zip_path = 'utils'
            try:
                zipf.write(utils_dir, utils_zip_path)
            except Exception as e:
                print(f"Warning: Could not add utils directory: {e}")
            
            # Add utils files
            if os.path.exists(utils_dir):
                for root, dirs, files in os.walk(utils_dir):
                    # Get the relative path from utils_dir
                    relpath = os.path.join('utils', os.path.relpath(root, utils_dir))
                    if relpath == 'utils\\' or relpath == 'utils/':
                        relpath = 'utils'
                ...
                print("Successfully included utils folder in the export")
            else:
                print(f"Warning: Utils directory not found at {utils_dir}")
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, 'r') as zipf:
            ...
            essential_files = [
                ...
                'terminal_games/utils_terminal.py',
                'terminal_games/roulette.py',
                'terminal_games/masterMind.py',
                'terminal_games/changelog.txt',
                'terminal_games/README.md'
            ]
...
def main():
    ...
    parser.add_argument(
        ...
        help='Include the utils folder in the export (for developer versions)'
    )
    parser.add_argument(
    ...
    # Create the zip file
    zip_path, games_dir_used = create_terminal_games_zip(version, args.games_dir, args.dev)
